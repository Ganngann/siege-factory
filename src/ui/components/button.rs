...
pub fn button_hover_system(
    mut query: Query<(&Interaction, &HoverableButton, &mut BackgroundColor), Changed<Interaction>>,
) {
    for (interaction, hoverable, mut bg) in query.iter_mut() {
        match *interaction {
            Interaction::Pressed => *bg = hoverable.pressed.into(),
            Interaction::Hovered => *bg = hoverable.hover.into(),
            Interaction::None => *bg = hoverable.inactive.into(),
        }
    }
}
