...
fn test_drag_start_via_interaction() {
    ...
    let mut w = app.world_mut().query_filtered::<&mut Window, With<PrimaryWindow>>();
    w.single_mut(app.world_mut()).expect("window")
        .set_cursor_position(Some(Vec2::new(200.0, 300.0)));

    let inv = app.world_mut().spawn(Inventory {
        resources: [(ResourceId("ore".into()), 5)].into(), capacity: 0,
    }).id();
    ...
    let grid = app.world_mut().spawn(InventoryGrid { cols: 5, rows: 1, owner: inv }).id();
...
fn test_drag_end_transfer_works() {
    ...
    let src = app.world_mut().spawn(Inventory {
        resources: [(ResourceId("iron_ore".into()), 5)].into(), capacity: 0,
    }).id();
    ...
    let grid = app.world_mut().spawn(InventoryGrid { cols: 1, rows: 1, owner: dst }).id();
    app.world_mut().entity_mut(grid).add_child(slot);
